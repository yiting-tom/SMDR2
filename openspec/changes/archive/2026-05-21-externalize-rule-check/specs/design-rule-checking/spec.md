...
**Reason**: Rule writing is owned by the external rule-checking
team. SMDR2 no longer ships its own mock for substrate-to-SMD
distance — that rule, along with its threshold and per-origin
sub-rule scoping, now lives entirely in the external team's
in-tree module. Keeping a parallel mock in our spec would let
the two drift.

**Migration**: Equivalent coverage is provided by the external
team's own tests against their in-tree module. SMDR2 retains
contract-level tests (adapter forwards the bundle path, validates
the envelope, persists the result) but no longer asserts on
rule-specific outputs. Consumers that previously depended on
Rule1's exact text or threshold SHOULD treat the external
module's rule-name choices as opaque and key off `pass` / `text`
/ `rules` instead.

### Requirement: Per-role bundle merging and handle prefix
**Reason**: The worker no longer merges per-role Match JSONs in
memory. It materialises the existing handoff bundle layout on disk
(per-file Match JSONs with raw, unprefixed handles) and hands the
directory path to the external rule function. The `<file_id[:8]>:`
prefix scheme was an internal artifact of the in-tree mock checker;
removing it deletes ~60 lines of worker code and aligns the live
path with the offline "Download All Match" handoff zip — same
bundle, different transport.

**Migration**: Callers of `app/rule_check.py` helpers
(`_first_match_handles`, `_all_match_groups`,
`_all_handles_for_prefix`, `_count_for_prefix`,
`_iter_class_groups`, `_shortest_distance`,
`_split_handle_prefix`) SHALL be removed from the tree along with
the helpers themselves — none have callers outside the deleted
mock rules. Tests that asserted on the prefix invariant SHALL be
rewritten to assert on the bundle directory layout instead (handles
in `match/<file_id>.json` are raw and unprefixed, which is already
covered by the "Match JSON handles are not pre-merged" scenario of
the "External DRC handoff bundle format" requirement).
