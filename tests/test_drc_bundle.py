...
These tests bind stub content rows into a real product+version via the
real `FILE_STORE` / `VERSION_STORE`, and write arbitrary bytes to
`uploads/{id}.dxf` + `match/{version_id}/{id}.json` — `build_bundle`
only byte-copies these into the zip, so the DXF bytes do not need to be
a valid DXF for the assembly assertions to mean anything. The
schema-validation and endpoint tests cover the same fixtures so we know
the manifest, the precondition checks, and the HTTP wrapper all stay in
lockstep.
"""
...
# Internal `<file_id[:8]>:` prefix the in-process role merge applies for
# the mock checker — MUST never leak into the exported bundle.
...
@pytest.fixture
def seeded_version():
    """Create a one-shot product + version + a `seed_role(...)` helper.

    The helper registers stub content, binds it into the version's role,
    writes the given bytes to disk for both the DXF and the Match JSON,
    and marks `match_saved = True`. Returns the binding FileRecord.
    """
    ...
    product, version = VERSION_STORE.create_product(pname, "v1")
    ...
    def seed_role(
        ...
        *,
        ...
        FILE_STORE.register_content(fid, f"{role}.dxf", len(dxf_bytes))
        FILE_STORE.bind(version.id, role, fid, dxf_view="multi")
        upload_path(fid).write_bytes(dxf_bytes)
        if match_json is None:
            match_json = {"substrate.0": [["7AF"]], "smd_2t.0": [["B01"]]}
        mp = match_path(version.id, fid)
        mp.parent.mkdir(parents=True, exist_ok=True)
        mp.write_text(json.dumps(match_json, indent=2))
        if match_saved:
            FILE_STORE.set_match_saved(version.id, fid, True)
        created_file_ids.append(fid)
        return FILE_STORE.get(version.id, fid)

    yield product, version, seed_role

    # Cleanup so we don't litter `uploads` + `match` + the DB.
    VERSION_STORE.delete_for_product(product.id)
    PRODUCT_STORE.delete(product.id)
    for fid in created_file_ids:
        upload_path(fid).unlink(missing_ok=True)
        match_path(version.id, fid).unlink(missing_ok=True)
        with FILE_STORE.lock, FILE_STORE.conn:
            FILE_STORE.conn.execute("DELETE FROM files WHERE id = ?", (fid,))


def _files_of(version) -> list:
    from app.files import FILE_STORE
    return [f for f in FILE_STORE.list_by_version(version.id) if f.dxf_role]
...
def test_build_bundle_single_role_validates_against_schema(
    ...
    zip_bytes, filename = build_bundle(product, version, _files_of(version))
    assert filename == f"drc-bundle-{product.id}-{version.id}.zip"
...
def test_build_bundle_multi_dxf_per_role(seeded_version, manifest_schema):
...
def test_build_bundle_carries_unit_fields_and_validates(
    ...
    FILE_STORE.update_parsed(
        version.id, rec.id, 0, (0.0, 0.0, 1.0, 1.0), "#ffffff",
...
def test_unit_fields_value_matrix(seeded_version, manifest_schema):
    ...
    def setup(role, *, insunits, applied_scale, override=None):
        ...
        if override is not None:
            FILE_STORE.set_user_unit_override(version.id, rec.id, override)
    ...
    manifest = build_manifest(product, version, _files_of(version))
    jsonschema.validate(manifest, manifest_schema)
...
def test_view_field_reflects_side_regions(seeded_version, manifest_schema):
    ...
    r_all = seed("SBT")
    FILE_STORE.update_side_regions(version.id, r_all.id, R, R, R)        # all three
    r_top = seed("BD")
    FILE_STORE.update_side_regions(version.id, r_top.id, R, None, None)  # only top
    r_bs = seed("POD")
    FILE_STORE.update_side_regions(version.id, r_bs.id, None, R, R)      # bottom + side
    seed("RING")                                                          # none set
...
def test_match_json_in_bundle_carries_raw_handles(seeded_version):
...
def test_dxf_and_match_entries_are_byte_for_byte_copies(seeded_version):
    ...
    files_list = _files_of(version)
    zip_bytes, _ = build_bundle(product, version, files_list)
    ...
    with _open_zip(zip_bytes) as zf:
        for rec in files_list:
            ...
            assert zip_match == match_path(version.id, rec.id).read_bytes()


# ---- 3.6 exported_at injection (out-of-order so the endpoint test can
#         lean on it for determinism) -----------------------------------
def test_exported_at_is_injectable(seeded_version):
    ...
    zip_bytes, _ = build_bundle(product, version, _files_of(version), now=frozen)
    manifest = _read_manifest(zip_bytes)
    assert manifest["exported_at"] == "2026-05-19T07:30:00Z"


# ---- 3.5 endpoint integration ------------------------------------------
def test_endpoint_returns_zip_on_happy_path(seeded_version, manifest_schema):
    ...
    with TestClient(app) as client:
        r = client.get(f"/api/versions/{version.id}/drc-bundle")
    assert r.status_code == 200
    assert r.headers["content-type"] == "application/zip"
    assert f"drc-bundle-{product.id}-{version.id}.zip" in r.headers["content-disposition"]
    ...
    assert manifest["version_label"] == version.label
    ...
    expected_roles = {f.dxf_role for f in _files_of(version)}
    assert {e["role"] for e in manifest["files"]} == expected_roles


def test_endpoint_404_for_unknown_version():
    from fastapi.testclient import TestClient
    from app.main import app

    with TestClient(app) as client:
        r = client.get("/api/versions/does-not-exist-xyz/drc-bundle")
    assert r.status_code == 404


def test_endpoint_400_when_match_saved_missing(seeded_version):
    """One BD binding with match_saved=False — endpoint must 400 and the
...
def test_endpoint_400_when_no_role_bound_files(seeded_version):
    from fastapi.testclient import TestClient
    from app.main import app

    product, version, _ = seeded_version   # no seed_role calls → no DXFs
...
def test_endpoint_works_on_signed_off_version(seeded_version):
    """Exporting a signed-off version's bundle is a READ — the freeze
    guard must not block it (C4: old versions stay reviewable)."""
    from fastapi.testclient import TestClient
    from app.main import app
    from app.versions import VERSION_STORE
    ...
    product, version, seed = seeded_version
    seed("BD")
    VERSION_STORE.sign_off(version.id, "tester")
...
def test_build_bundle_lid_configuration_validates(seeded_version, manifest_schema):
    """A version in the LID configuration (LID instead of RING) emits
...
def test_build_bundle_includes_both_ring_and_lid(seeded_version, manifest_schema):
    """RING and LID are independent roles: a version holding files
...
def test_manifest_raises_on_product_version_mismatch(seeded_version):
    """If the version handed in does not belong to the product,
    `build_manifest` MUST raise `ValueError` — silently emitting a
    cross-wired manifest would corrupt the external team's contract."""
    from app.drc_bundle import build_manifest
    from app.products import PRODUCT_STORE
    from app.versions import VERSION_STORE
    ...
    other_p, other_v = VERSION_STORE.create_product("mismatch-probe", "v1")
    try:
        with pytest.raises(ValueError, match=version.id):
            build_manifest(other_p, version, files_list)
    finally:
        VERSION_STORE.delete_for_product(other_p.id)
...
def test_build_bundle_dir_matches_zip_contents(seeded_version, tmp_path):
    ...
    zip_bytes, _ = build_bundle(product, version, files_list, now=frozen)
    bundle_dir = build_bundle_dir(product, version, files_list, tmp_path, now=frozen)
...
def test_materialise_bundle_cleans_up_after_context(seeded_version):
    ...
    captured: list[Path] = []
    with materialise_bundle(product, version.id, files_list) as bundle_dir:
